...
def test_entry_point():
    ...
    invalid_primitive = (
        'Featuretools failed to load "invalid" primitives from "featuretools_primitives.invalid_primitive". '
        + "For a full stack trace, set logging to debug."
    )
    assert invalid_primitive in featuretools_log

    existing_primitive = (
        'While loading primitives via "existing" entry point, '
        + 'ignored primitive "Sum" from "featuretools_primitives.existing_primitive" because a primitive '
    )
